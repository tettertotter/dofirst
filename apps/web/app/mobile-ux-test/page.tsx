    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <Card>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '16px' }}>
          <StatusCard
            title="Connection"
            value={mounted ? connection.quality : 'Loading...'}
            color={mounted ? getQualityColor(connection.quality) : '#6b7280'}
            icon="📡"
          />
          <StatusCard
            title="Battery"
            value={mounted && battery.level ? `${Math.round(battery.level * 100)}%` : 'N/A'}
            color={mounted && battery.isCharging ? '#10b981' : getBatteryColor(battery.level)}
            icon={mounted && battery.isCharging ? '🔌' : '🔋'}
          />
          <StatusCard
            title="Performance"
...
function PerformanceTab() {
  const handleMetric = React.useCallback((metric: any) => {
